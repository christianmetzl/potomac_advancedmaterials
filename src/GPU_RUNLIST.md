...
| 4 | Real QPU 12q H₆ QSCI (P5) | `python src/qbraid_submit.py --device openquantum:ionq:qpu:forte-1 --shots-per-job 3334 --yes` (or `--device aws:ionq:qpu:forte-1`, ≈80k credits ≈ $801; openquantum channel listed at 0 credits — confirm before relying on it) | FCI (exact) | ≤5 mHa; raw counts committed regardless | ✅ **entire chain executed through the qBraid cloud runtime** (free qir-sv tier, 0 credits): +2.0/+2.4 mHa PASS, job IDs in `results/qbraid_P5_*_evidence.json`. Export verified per-artifact (L1<0.01; qml.to_openqasm found unfaithful at large angles and bypassed); count decoding proven by 4 binary probe circuits; spend-guarded (`--estimate`, `--yes`) |
