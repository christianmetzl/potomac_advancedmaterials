"""SnO (16 qubits) via CASCI(8,8) with def2-ECP on Sn -> qubit Hamiltonian -> QSCI.
...
t0=time.time()
# --- Build SnO with effective core potential on Sn ---
R=1.833  # SnO equilibrium bond length (Angstrom)
mol=gto.M(atom=f"Sn 0 0 0; O 0 0 {R}", basis={'Sn':'def2-svp','O':'def2-svp'},
          ecp={'Sn':'def2-svp'}, verbose=0, spin=0, charge=0)
mf=scf.RHF(mol).run(conv_tol=1e-10)
print(f"SnO: RHF={mf.e_tot:.6f} | {mol.nao} orbitals, {mol.nelectron} electrons (Sn ECP applied) | {time.time()-t0:.0f}s")
...
print(f"SnO CAS(8,8): {nq} qubits, {len(qop.terms)} Pauli terms, FCI(CASCI)={e_fci:.6f}")

# --- QSCI (selected-CI) on the SnO Hamiltonian ---
XM=[];ZYM=[];PH=[]
...
for it in range(8):
    ...
    if err<0.5 or len(space)>=2000: break
...
print(f"\nSnO QSCI final error vs FCI: {abs(E-e_fci)*1000:.3f} mHa | total {time.time()-t0:.0f}s")
