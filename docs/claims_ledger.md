...
| 20b | **EUV-motif failure-catch (Sn₂O₂ bridge cleavage)** | on the real tin-oxo rhombus, Sn–O bridge stretch 2.05→3.28 Å: in-CAS CCSD(T) error grows **0.14→5.49 mHa (~40×, crosses chem-acc between 2.36–2.67 Å)** while QSCI stays variational **≤0.47 mHa** throughout; dominant-det weight collapses **0.95→0.53** (strong-correlation onset) — the failure-catch now on EUV chemistry, in the strongly-correlated regime where resist cleavage lives, not just at equilibrium | `src/sn2o2_dissociation.py` | `sn2o2_dissociation_evidence.json` | PASS | executed-CPU (CASCI ref) |
| 21b | **Candidate ranking-flip decision (CrO vs NiO)** | rank two real oxide centers by high-spin preference (spin-gap, eV): QSCI/CASCI give CrO **+1.89** > NiO **+1.66** eV -> synthesize CrO (agrees with CrO exp. X 5-Pi). 5/6 DFT functionals agree; **B3LYP INVERTS the ranking** (CrO gap -0.076 eV sign-error) -> a B3LYP-only screen advances the WRONG candidate. Frozen decision rule stated pre-run; multireference computed then rule evaluated as-measured | `src/candidate_decision.py` | `candidate_decision_evidence.json` | AUDIT-able (offline) | **executed-CPU** (CAS(10,10), def2-SVP) |
