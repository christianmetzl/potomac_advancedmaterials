...
- **Real oxide (CrO bond stretch, CAS(10,10)=20q):** in-active-space CCSD(T) error grows to **~140 mHa and
...
| **p** — silent-failure rate | fraction of **multireference** candidates where the classical gold standard is confidently wrong with no warning | *measured mechanism here:* CCSD(T) non-variational collapse (to ~140 mHa on CrO; below FCI on H₁₀); the rate over your library is yours to measure |
