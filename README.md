...
- **Transfer learning, reported honestly.** Cross-*size* transfer **works at 16q and 20q**: via a canonical frontier-relative tokenization, a generator trained on small systems beats random search on a larger unseen one — H₆→H₈ raw energy (mean 91.8 vs 127.6 mHa) and H₄+H₆→H₁₀/20q at a matched determinant budget (QSCI 58.7 vs 73.9 mHa), all seeds (`src/encoder/scaling_transfer.py`). Direct evidence for scalability. Cross-*molecule* conditioning (same size) was a within-noise **tie**, reported as a negative (`src/encoder/decisive_transfer.py`). Innovation leads with the integrated MPS + QSCI + operator-pool-compression scaling layer plus this cross-size transfer result.
