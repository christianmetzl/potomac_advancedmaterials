...
const children=[
  ...
  P([T("Transfer learning — a generative prior that transfers across system size and chemistry. ",{b:true}),T("Using a canonical, frontier-relative tokenization (each excitation encoded as occupied-depth-below-HOMO and virtual-height-above-LUMO, independent of qubit count), a small system's token vocabulary is a provable subset of a larger one (H₆/12q ⊂ H₈/16q ⊂ H₁₀/20q). "),T("One GPT-QE generator trained only on H₄+H₆ (8q+12q) is deployed zero-shot across 16→56 qubits and, on the 3-seed mean, proposes determinant subspaces that beat random selection at every size",{b:true}),T(" — mean advantage +8.9/+8.3/+7.1/+5.8/+5.0/+4.4 mHa at 16/20/28/40/48/56q. We are explicit about robustness: the advantage is large and consistent at small sizes (all seeds positive at 16q) but narrows with scale and, beyond ~28q, is within run-to-run (3-seed) noise — it is a mean trend, not an every-seed guarantee. This is the "),T("train-small, deploy-large",{i:true}),T(" idea: GQE training is the cost that is CPU-bound near ~16 qubits, so transferring a small-trained generator toward the 40q+ target — instead of training at scale — speaks directly to the scalability criterion. The enabler is a determinant-space evaluation (each proposed excitation maps by bitmask to a determinant; the Hamiltonian is diagonalized in that subspace via Slater-Condon, validated to 0.0000 mHa against the Jordan–Wigner engine) that needs no 2ⁿ statevector, so 56q runs on CPU. This is a "),T("selected-CI proxy",{i:true}),T(" for circuit-sampled QSCI (deterministic determinant construction, no shots), not executed QSCI. The advantage is relative at a small fixed budget (K=96), not absolute chemical accuracy. ")]),
  P([T("Cross-chemistry transfer (honest, partial). ",{b:true}),T("The same H-chain-trained generator, deployed to oxide active spaces (CO, SiO, BeO, SnO at 12q; CO, SnO at 20q), beats random on 4 of 6 targets (CO/SiO/CO-20q by +3–4 mHa, BeO a tie) but loses on SnO at both sizes (−2.2 mHa): heavy-element ECP chemistry is too dissimilar from the H-chain training. Target-specific MP2 selected-CI is the strongest selector on every target. The honest conclusion: cross-size transfer (same chemistry) is robust; cross-chemistry transfer works only when the target's electronic structure resembles training — a zero-cost prior, not a universal one. A same-size cross-molecule conditioning variant gave only a within-noise gain and is also reported as a negative.")]),

  new Paragraph({spacing:{before:50,after:10,line:240,lineRule:"auto"},alignment:AlignmentType.CENTER,
    children:[new ImageRun({type:"png",data:fs.readFileSync("../results/encoder/scaling_ladder.png"),transformation:{width:300,height:184}})]}),
  new Paragraph({spacing:{before:0,after:60,line:240,lineRule:"auto"},alignment:AlignmentType.CENTER,
    children:[new TextRun({text:"Figure 2. Train-small, deploy-large. A generator trained only on 8- and 12-qubit systems proposes lower-energy determinant subspaces than random selection at every target size up to 56 qubits, on the 3-seed mean (lower is better; determinant-space selected-CI / QSCI-proxy, matched budget K=96). Trained energies are tightly consistent across seeds (spread ~2 mHa); the mean advantage persists to 56q while narrowing into run-to-run noise at the largest sizes.",font:FONT,size:18,italics:true})]}),
  ...
  P([T("Determinant-budget sweep and a classical selected-CI baseline. ",{b:true}),T("Sweeping the selected-CI (QSCI-proxy) determinant budget K at 20q and 40q confirms the transfer advantage is budget-robust: the zero-shot generator beats random selection at essentially every K (e.g. 20q, K=64: 60.0 vs 73.2 mHa; 40q, K=64: 179.9 vs 189.4 mHa), converging to parity only at the largest 20q budget (K=128). As a classical baseline we add MP2-ranked selected-CI on the same determinant subspaces: using the target's own MP2 amplitudes it is the strongest selector (20q, K=64: 35.9 mHa), and a reciprocal-rank fusion of generator+MP2 does not improve on MP2 alone — the two signals are largely redundant. The honest reading: the transferred generator provides selection between random and target-specific MP2 at "),T("zero target-specific cost",{i:true}),T(" (a strong zero-shot prior for high-throughput screening), while MP2 selected-CI is the appropriate classical comparator where its per-candidate cost is affordable.")]),
  ...
  P([T("Where it matters most: trust under strong correlation. ",{b:true}),T("Equilibrium benchmarks understate the problem — they live where classical methods already work. Stretching H₁₀ (20q) to dissociation drives genuine multireference character (Hartree–Fock weight in the exact wavefunction falls 0.93→0.06), and the classical gold standard fails qualitatively: CCSD(T) collapses "),T("non-variationally",{i:true}),T(" to 217 mHa BELOW the exact FCI energy at R=2.4 Å — a confidently wrong, unphysical answer. The determinant-subspace method (the QSCI principle, here via iterative selected-CI) instead "),T("reaches chemical accuracy with only ~500 determinants",{b:true}),T(" at every geometry across the dissociation, remaining a rigorous variational bound that is systematically convergent to the exact answer. This is the core value proposition: in the regime that matters for open-shell metal-oxide chemistry, the subspace approach is trustworthy where the classical gold standard is not. (Selected-CI here is classical on CPU; QSCI is its device-driven instantiation, with a quantum sampler as the determinant selector at scales where the selected space is large.) "),Q("[QBRAID-RUN: device-sampled selection at scale]")]),
  P([T("Proxy validated against real measurement; cost scaling. ",{b:true}),T("The scaling ladder uses a determinant-space selected-CI proxy; we confirm it tracks the real circuit-sampled pipeline by executing the measured QSCI (qml.sample, 3,000 shots × 160 circuits) at 16q and 20q — the measured energies match the proxy (16q: 26.6 vs 28.1 mHa; 20q: 49.3 vs 50.4 mHa), extending the genuinely-sampled pipeline beyond the prior 12-qubit point. The determinants needed for chemical accuracy grow ~1.38× per qubit versus 2.0× for the full FCI space (log-space R²=0.99) — a vanishing fraction (75% at 8q → 0.15% at 28q), though still exponential: at 40q+ the selected space reaches ~10⁶ determinants, the regime where device-driven selection becomes valuable. We do not claim polynomial scaling.")]),
