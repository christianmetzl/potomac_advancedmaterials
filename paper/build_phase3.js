...
const children=[
  ...
  P([T("Transfer learning — generative structure transfers across system size. ",{b:true}),T("Using a canonical, frontier-relative tokenization (each excitation encoded as occupied-depth-below-HOMO and virtual-height-above-LUMO, independent of qubit count), a small system's token vocabulary is a provable subset of a larger one (H₆/12q ⊂ H₈/16q ⊂ H₁₀/20q). "),T("A GPT-QE generator trained only on H₆ (12 qubits) generates zero-shot for H₈ (16 qubits) and beats random search across all three seeds — mean 91.8 vs 127.6 mHa, best 67.4 vs 82.9 mHa",{b:true}),T(" (within-seed spread ~1 mHa against a ~36 mHa gap). The result extends to "),T("H₁₀ (20 qubits): a generator trained on H₄+H₆ (8q+12q) selects better determinants for H₁₀ than random at a matched determinant budget — QSCI 58.7 vs 73.9 mHa across three seeds",{b:true}),T(" (at equal determinant count, controlling for sampling diversity). The learned policy generalizes to larger, unseen systems — direct evidence for scalability. (Honest notes: at equal sampling, an untrained random search pools more distinct determinants, so a diversity-uncontrolled metric can favor it — the matched-budget comparison isolates determinant quality. A separate same-size cross-molecule conditioning variant gave only a within-noise gain — reported as a negative; the value is in cross-size transfer.)")]),
